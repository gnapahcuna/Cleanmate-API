...
header("Content-type: text/plain; charset=utf-8");
	...
	$mOrderNo="";
	$mBranchID="";
	$stmt = "select OrderNo,BranchID from ops_transportpackage where OrderNo='".$_GET['OrderNo']."' AND BranchID='".$_GET['branchID']."' AND DeliveryStatus=1 AND IsDriverVerify=1 AND IsCheckerVerify=1
    ...
    if($row = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC))
    {
		...
		$sql_2 = "update ops_transportpackage SET IsBranchEmpVerify =?,BranchEmpVerifyDate =? where OrderNo=?";
			$params_2 = array($_GET['verify'],$_GET['date'],$mOrderNo);
