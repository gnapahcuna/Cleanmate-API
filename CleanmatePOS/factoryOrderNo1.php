<?php
    ...
    $stmt = "SELECT DISTINCT ops_transportpackage.OrderNo,
				...
				FROM ops_transportpackage left join ops_order on ops_transportpackage.OrderID=ops_order.OrderID left join uac_customer on ops_order.CustomerID = uac_customer.CustomerID where ops_order.BranchID='".$_GET['branchID']."' AND (ops_transportpackage.OrderNo LIKE '%".$_GET['search']."%' OR uac_customer.TelephoneNo LIKE '%".$_GET['search']."%') AND ops_order.IsActive='1'";
