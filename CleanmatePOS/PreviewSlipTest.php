<?php
	...
	$stmt1="select coalesce(count(distinct cd.CouponID),0) as CouponID from (ops_order od left join ops_orderdetail odd on od.OrderNo=odd.OrderNo) 
left join ops_coupondiscount cd on od.OrderNo=cd.OrderID where od.OrderNo='".$_POST['orderNo']."'";
    ...
    $object_array = array();
	$couponID='0';
	if($row1 = sqlsrv_fetch_array($query1, SQLSRV_FETCH_ASSOC))
    {
	...
	for($i=0;$i<sizeof($array1);$i++){
		...
		'phone_customer'=>$array16[$i],'BranchNameTH'=>$array17[$i],'BranchCode'=>$array18[$i],'phone_branch'=>$array19[$i],'IsPayment'=>$array20[$i],'IsExpressLevel'=>$array21[$i],'CustomerType'=>$array22[$i],'PaymentType'=>$array23[$i],
		'PaymentCash'=>$array24[$i],'counts'=>$array25[$i],'total'=>$array26[$i],'AdditionAmount'=>$array27[$i],'IsCancel'=>$array28[$i],'IsCancelService'=>$array29[$i],'DiscountAmount'=>$array30[$i]));
	}
