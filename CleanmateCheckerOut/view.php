...
header("Content-type: text/plain; charset=utf-8");
	...
	$stmt3 = "select SubOrderDetailID from ops_subprocess where SubOrderDetailID='".$SubOrderDetailID."'";
    $query3 = sqlsrv_query($conn, $stmt3);
	if($result = sqlsrv_fetch_array($query3, SQLSRV_FETCH_ASSOC))
	{
		$check=$result['SubOrderDetailID'];
	}else{
		$check=0;
	}
	
	if($check!=0&&$OrderDetail!=0){
		$sql = "insert into ops_subprocess (SubProcessID,SubOrderDetailID,EmpVerifyID,EmpVerifyDate) values (?,?,?,?)";
